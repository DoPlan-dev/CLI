...
## Action
The `/state` helper wraps `go run scripts/statehistory/main.go` so you can manage `.plan/active_state.json` history safely.
...
## Agent Involvement
- **Project Orchestrator**: Maintains accurate project state
- **QA Engineer**: Verifies rollback actions
