...
func TestNewProjectModel_Initialization(t *testing.T) {
	...
	assert.Equal(t, 5, len(model.installSteps))
}
...
func TestNewProjectModel_CreateProjectStructure(t *testing.T) {
	...
	// Check .doplan directories were created
	// Note: doplan/contracts and doplan/templates are now created by generateInitialFiles()
	assert.DirExists(t, filepath.Join(projectRoot, ".doplan"))
	assert.DirExists(t, filepath.Join(projectRoot, ".doplan", "ai", "agents"))
	assert.DirExists(t, filepath.Join(projectRoot, ".doplan", "ai", "rules"))
	assert.DirExists(t, filepath.Join(projectRoot, ".doplan", "ai", "commands"))
}
